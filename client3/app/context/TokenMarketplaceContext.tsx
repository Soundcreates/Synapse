...
export const TokenMarketPlaceContext = createContext<
  TokenMarketplaceContextType | undefined
>(undefined);

export const TokenMarketPlaceProvider = ({
  children,
}: {
  ...
  const [mkpContract, setMkpContract] = useState<Contract>({
    ...
    abi:
      typeof TokenMarketPlace.abi === "string"
        ? JSON.parse(TokenMarketPlace.abi)
  ...
  useEffect(() => {
    ...
    const initContracts = async () => {
      ...
      try {
        ...
        toast({
          ...
          description:
            "Failed to initialize contracts. Please check your wallet connection.",
          variant: "destructive",
        });
        setLoading(false);
      }
    ...
    const initContractsWithProvider = async (
      provider: ethers.BrowserProvider,
    ) => {
      try {
        // Initialize SynTK contract
        const parsedAbi =
          typeof SynTK.abi === "string" ? JSON.parse(SynTK.abi) : SynTK.abi;
        const synTKInstance = new ethers.Contract(
          SynTK.address,
          parsedAbi,
          provider,
        );
        setTokenContract((prev) => ({
          ...prev,
          contractInstance: synTKInstance,
        }));
  ...
  const fetchBalance = async (receiverAddress: string): Promise<string> => {
    ...
    try {
      ...
      try {
        ...
        if (code === "0x") {
          console.error(
            `No contract deployed at address ${tokenContract.address} on Sepolia testnet`,
          );
          toast({
            title: "Contract Not Found",
            description:
              "The token contract is not deployed on the current network.",
            variant: "destructive",
          });
          return "0";
        }
      ...
      try {
        ...
        if (
          balanceErr.code === "BAD_DATA" ||
          balanceErr.reason === "BAD_DATA"
        ) {
          console.error(
            "BAD_DATA error - possible ABI mismatch or contract not deployed",
          );
          toast({
            title: "Contract Error",
            description:
              "There's an issue with the contract deployment or ABI. Please check the network and contract address.",
            variant: "destructive",
          });
          return "0";
        }
      ...
      if (!err.message?.includes("Wrong network")) {
        toast({
          title: "Error",
          description:
            "Failed to fetch token balance. Please check your network connection and try again.",
          variant: "destructive",
        });
      }
      return "0";
  ...
  const buyTokens = async (amount: number, receiverAddr: string) => {
    ...
    try {
      ...
      console.log(
        `Buying ${amount} tokens for receiver ${receiverAddr} at cost ${ethers.formatEther(cost.toString())} ETH`,
      );
